public class Store {
    ...
    public static void writeResultAlltoall(String outLocation, int sizeDados, int numRept, int numProcs, long timeMin, long timeMax, long timeAvg, long timeSpawn) {
        ...
        String result = String.format("%-9d\t %-14d\t %-14d\t %-16d\t %-16d\t %-16d\t %-16d\t%n",
                                      sizeDados, numRept, numProcs, timeMin, timeMax, timeAvg,timeSpawn);

        writeResult(outLocation, header, result);
    }

    public static void writeResultMulti(String outLocation, int sizeDados, int numProcs, int numRept, long timeExec, long timeSpawn) {
        String header = String.format("%-13s %-13s %-12s %-16s %-12s %n", "bytes", "processos", "repeticoes", "tempo exec[usec]", "tempo de spawn[usec]");
        String result = String.format("%-13d\t %-12d\t %-12d\t %-15d\t %-20d\t %n", sizeDados, numProcs, numRept, timeExec, timeSpawn);

        writeResult(outLocation, header, result);
    }

    public static void writeResultPeer(String outLocation, int sizeDados, int numRept, long timeExec, long timeSpawn) {
        String header = String.format("%-13s\t %-12s\t %-15s\t %-12s\t %n", "bytes", "repeticoes", "t[usec]", "tempo de spawn[usec]");
        String result = String.format("%-13d\t %-12d\t %-14d\t %-20d\t %n",sizeDados, numRept,timeExec,timeSpawn);
    ...
    private static boolean concatenar(String local, String content) {
        InputFile fileIn = new InputFile();
        if(fileIn.open(local)) {
